...
public class ImageNode {
    ...
    public boolean isEmpty(){
        return (next == null);  //is next empty
    }
    ...
    private int countIter() {
        /*# YOUR CODE HERE */
        int count = 0; //count at zero
        ImageNode node = this; //node is this
        while(node != null){ //iterate through
            count++; //add count
    ...
    private int countRec() {
        ...
        return 1+(this.getNext().countRec()); //recursively call add the count
    }
    ...
    public void insertBefore(ImageNode newNode, ImageNode cursor) {
        /*# YOUR CODE HERE */
        ImageNode previous = nodeBefore(cursor);
    ...
    public ImageNode nodeBefore(ImageNode target) {
        if (SlideshowApp.isRecursive) {
            ImageNode node = this;
            return nodeBeforeRec(target, node);
        } else {
    ...
    private ImageNode nodeBeforeIter(ImageNode target) {
        /*# YOUR CODE HERE */
        ImageNode node = this; //set node to this
        while(node.getNext() != target){  //iterate through to find target   
            node = node.getNext();         
        }
        return node; //return the pervious node
    }
     ...
     * @param target the node whose predecessor is required 
     */
    private ImageNode nodeBeforeRec(ImageNode target, ImageNode previous) {
        /*# YOUR CODE HERE */ 
        if(previous.getNext() == target) return previous; //if equal to traget return previous
        return nodeBeforeRec(target, previous.getNext()); //recursive
    }
    ...
    public ImageNode reverseUsingPrevious(ImageNode previous) {
        /*# YOUR CODE HERE 
         * This method was not needed in my code   */
        if(previous.getNext() == null){
            return previous;
        }
        reverseUsingPrevious(previous.getNext());
