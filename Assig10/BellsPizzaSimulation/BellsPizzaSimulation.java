...
public class BellsPizzaSimulation {
    ...
    /*# one line to change the number of cyclists */
    private static final int numberOfCyclists = 3; 
    ...
    private double run (Queue<Pizza> queue, Scanner logFile) {
        ...
        MinHeapArrayQueue<Cyclist> cyclists = new MinHeapArrayQueue<>(); //new queue of cyclists
        for(int i = 0; i < numberOfCyclists; i++){//loop throught number of cyclists wanted
            Cyclist cyclist = new Cyclist(); //new cyclist object
            cyclists.offer(cyclist); //add cyclist to queue
        }
        ...
        while (logFile.hasNext()) {
            ...
            if (time>cyclists.peek().getReturnTime() && !queue.isEmpty()) { //peek at current cyclist time
                ...
                Cyclist c = cyclists.poll(); //poll the cyclist of the queue
                 
                c.setReturnTime(time + fullTripTime); //set new returntime
                cyclists.offer(c);//add cyclist to back of the queue
