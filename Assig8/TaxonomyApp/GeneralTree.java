...
public class GeneralTree
{
    ...
    public void addNode(String newName, String parentName) {
        /*# YOUR CODE HERE */ 
        GeneralTreeNode newChild = new GeneralTreeNode(newName); //new node
        if(parentName == null){
            if(root != null){
                GeneralTreeNode child = root; //if the root exsists it now becomes the child of the new root
                root = new GeneralTreeNode(newName); //new root
                root.addChild(child); //add the new child
            }else{
                root = new GeneralTreeNode(newName); //new tree
            }
        }else{
            if(root.contains(newChild)){ //check if the node exists
                UI.println(newName+" is already in the Taxonomy");
                return;
            }

            GeneralTreeNode parent = findNode(parentName); //find the parent
            if(parent == null) return;
            parent.addChild(newChild); // add the parent
        }
    ...
    public void removeNode(String targetName) {
        /*# YOUR CODE HERE */ 
        if(root == null) return; //base case checks
        if(root.getName().equals(targetName)) return;
        
        GeneralTreeNode temp = findNode(targetName); 
        if(temp == null) return;
        GeneralTreeNode parent = temp.getParent(); //set parent
        parent.addChildrenFromNode(temp); //add the children from the target node
        temp.remove(); //remove the target node
    ...
    public void moveSubtree(String targetName, String destinationName) {
        ...
        if(targetNode == null || destinationNode == null) return; //check it null
        if(targetNode.contains(destinationNode)){ //check where you are moving the subtree
            UI.println("Cannont move " + targetNode.getName() + " into it's own subtree");
            return;
        }
        GeneralTreeNode temp = targetNode; //set a temp
        temp.remove(); //remove it
        destinationNode.addChild(targetNode); //add the connection
    }
    ...
    public String findClosestCommonAncestor(String target1, String target2) {
        ...
        Set<GeneralTreeNode> ancestor = new HashSet<GeneralTreeNode>(); //set of ancestors
        if(targetNode1 == null || targetNode2 == null) return null; //null check
        if(target1.equals(target2)) return target1; //base case
        while(targetNode1 != null){
            ancestor.add(targetNode1);    //add to the set
            targetNode1 = targetNode1.getParent();   //iterate up the tree   
        }
        while(targetNode2 != null){
            if(ancestor.contains(targetNode2)) return targetNode2.getName(); //founc the closest ancestor
            targetNode2 = targetNode2.getParent(); //iterate up the tree
        }
        return root.getName() ; //it must be the root
    }
    ...
    private void calculateLocations() {
        ...
        for (int d=0; d<100 && widths[d] !=0 ; d++) {
            separations[d] = (UI.getCanvasWidth()-20) / (widths[d]); /*# Set the width to be in the middle */ 
        }
    ...
    private void setLocations(GeneralTreeNode node, int depth, int[] nextPos, int[] separations) {
        node.setLocation(new Location(nextPos[depth], levelSep*depth+levelSep/3));
    ...
    public void redraw() {
        ...
        if (root == null)
            return;
        int level = 0; //set level for colour coding
        calculateLocations();
        root.redrawSubtree(level);
    }
}
